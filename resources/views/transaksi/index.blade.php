...
@section('content')
  <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
      <a href="{{ route('') }}" class="button text-white bg-theme-1 shadow-md mr-2">Add New Product</a>
        ...
        <tbody>
          @foreach ($products as $index => $product)
            <tr class="intro-x">
              ...
              <td class="text-center w-12">{{ number_format($product->jumlah, 0, ',', '.') }}</td>
              <td class="text-center w-20">IDR {{ number_format($product->harga, 0, ',', '.') }}</td>
              <td class="text-center w-64">{{ Str::limit(html_entity_decode(strip_tags($product->deskripsi)), 20) }}</td>
